...
//     $env:ETH_RPC_URL="http://127.0.0.1:8545"
//     $env:SENDER_PRIVATE_KEY="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
//     go run main.go --mode transfer --contract 0x5FbDB2315678afecb367f032d93F642f64180aa3 --to 0x70997970C51812dc3A010C7d01b50e0d17dc79C8 --amount 2.8
//
//  3. 发送 ERC-20 转账交易（使用代币的最小单位）：
//     $env:ETH_RPC_URL="http://127.0.0.1:8545"
//     $env:SENDER_PRIVATE_KEY="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
//     go run main.go --mode transfer --contract 0x5FbDB2315678afecb367f032d93F642f64180aa3 --to 0x70997970C51812dc3A010C7d01b50e0d17dc79C8 --amount 1500000
//
//  4. 解析交易中的 Transfer 事件：
//     $env:ETH_RPC_URL="http://127.0.0.1:8545"
//     go run main.go --mode parse-event --tx 0xd880b7504254bd7cb9b580a84cdc75e19e90d429b9ead5b7313bca6f1144add9
//
// 注意事项：
...
func handleBalanceOf(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex, addrHex string) {
	...
	contractAddr := common.HexToAddress(contractHex) // 合约地址
	targetAddr := common.HexToAddress(addrHex)       // 账户地址
...
func handleTransfer(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex, toHex, amountStr string) {
	...
	fromAddr := crypto.PubkeyToAddress(*publicKeyECDSA) // 发送方地址
	contractAddr := common.HexToAddress(contractHex)    // 合约地址
	toAddr := common.HexToAddress(toHex)                // 接收方地址
	...
	txData := &types.DynamicFeeTx{
		ChainID:   chainID,
		Nonce:     nonce,
		GasTipCap: gasTipCap,     // 给矿工/验证者的最大小费，即优先费上限。
		GasFeeCap: gasFeeCap,     // 为每单位 gas 付出的最高总价格，（base fee：网络基础费，销毁  + tip：给矿工/验证者的小费）
		Gas:       gasLimit,      // 最多愿意消耗多少单位的 gas
...
func waitForTransaction(ctx context.Context, client *ethclient.Client, txHash common.Hash) {
	...
	for {
		...
		case <-waitCtx.Done():
			...
			return
		case <-ticker.C:
			receipt, err := client.TransactionReceipt(waitCtx, txHash) // 查询交易回执
			if err != nil {
				// 交易可能还在 pending
				continue
			}
